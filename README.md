...
# options
- `git_gutter_leave_space`: a `bool` that, if `true`, will cause there to always be a column dedicated to the git gutter when the highlighter is active, even if there is no diff between the buffer and the staged file.
